...
const SuppliesUsedCard = memo(({ data, onScan, offline }) => {
  const [searchQuery, setSearchQuery] = React.useState('');
  const [isExpanded, setIsExpanded] = React.useState(false);
  const suppliesData = Array.isArray(data) ? data : [];

  const renderUsedItems = () => {
    const filteredItems = suppliesData.filter(item =>
    ...
    if (filteredItems.length === 0) {
      return (
        <View style={styles.emptyState}>
          <Icon name="package-variant" size={48} color="#2196f3" />
          <Text style={styles.emptyText}>
            {searchQuery ? 'No matching supplies found' : 'No supplies used yet'}
          </Text>
          <Text style={styles.emptySubtext}>
            Scan a supply QR code to add it to this list
          </Text>
        </View>
      );
    }
      ...
      <View style={styles.itemsContainer}>
        ...
        >
          {itemsToShow.map((item, index) => (
            <Surface key={`supply-${item.id || index}-${item.timestamp || index}`} style={styles.itemCard} elevation={2}>
              <View style={styles.itemContent}>
                <View style={styles.itemLeft}>
                  <View style={styles.itemIcon}>
                    <Icon name="package-variant" size={20} color="#2196f3" />
                  </View>
                  <View style={styles.itemInfo}>
                    <Text style={styles.itemName} numberOfLines={2}>{item.name || 'Unknown Supply'}</Text>
        ...
        <View style={styles.headerContent}>
          <View style={styles.headerLeft}>
            ...
            <View style={styles.headerText}>
              <Text style={styles.headerTitle}>Supplies Used</Text>
              <Text style={styles.headerSubtitle}>
                {suppliesData.length} {suppliesData.length === 1 ? 'item' : 'items'} • Medical supplies inventory
              </Text>
            </View>
          </View>
      ...
      <View style={styles.cardContent}>
        ...
        {suppliesData.length > 0 && (
          <View style={styles.searchContainer}>
            <Searchbar
              placeholder="Search supplies by name or description..."
        ...
        <View style={styles.actionContainer}>
          ...
          >
            <Text style={styles.buttonText}>
              {offline ? 'Offline - Cannot Scan' : 'Scan Supply QR Code'}
            </Text>
          </Button>
        </View>
